//! crates/pc_core/src/waveguide.rs
...
impl Waveguide {
    ...
    pub fn new_from_table_i(
        pc_definition: PhotonicCrystal,
        pc_background_material: Material,
    ) -> Self {
        Self {
            layers: vec![
                LayerType::Simple {
                ...
                LayerType::PhotonicCrystal {
                    ...
                    thickness: 0.1180e-6,
                    pc_definition,
                    background_material: pc_background_material,
                },
                LayerType::Simple {
                    name: "GaAs".into(),
                    thickness: 0.0590e-6,
    ...
    pub fn pc_layer_index(&self) -> Option<usize> {
        self.layers
            .iter()
    ...
    pub fn get_pc_layer(&self) -> Option<&LayerType> {
...
mod tests {
    ...
    fn test_waveguide_creation_with_material_pc() {
        let (pc_geom, gaas) = create_test_pc();
        let wg = Waveguide::new_from_table_i(pc_geom.clone(), gaas);
        ...
        match &wg.layers[2] {
            LayerType::PhotonicCrystal { pc_definition, background_material, .. } => {
                assert_eq!(
                    pc_definition.lattice.lattice().a1,
                    pc_geom.lattice.lattice().a1
                );
                assert_eq!(pc_definition.base.atoms.len(), 1);
    ...
    #[test]
    fn test_pc_layer_index_and_getter() {
        let (pc_geom, gaas) = create_test_pc();
        let wg = Waveguide::new_from_table_i(pc_geom, gaas);

        assert_eq!(wg.pc_layer_index(), Some(2));
        
        let retrieved_pc_layer = wg.get_pc_layer().unwrap();
        match retrieved_pc_layer {
            LayerType::PhotonicCrystal { pc_definition, .. } => {
                 assert_eq!(pc_definition.lattice.lattice().a1, (295e-9, 0.0, 0.0));
            }
            _ => panic!()
        }
    }
